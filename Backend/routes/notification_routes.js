...
router.post('/notifications/send', isAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.request()
      ...
      .input('event_date', sql.DateTime, req.body.event_date ? new Date(req.body.event_date) : null)
      .query(`
        INSERT INTO notifications (uid, title, message, type, send_to_all, created_by, event_date)
        VALUES (@uid, @title, @message, @type, @send_to_all, @created_by, @event_date);
...
router.post('/notifications/send-all', isAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        INSERT INTO notifications (uid, title, message, type, send_to_all, created_by, event_date)
        VALUES (NULL, @title, @message, @type, @send_to_all, @created_by, @event_date);
        SELECT @@IDENTITY as id;
      `);
